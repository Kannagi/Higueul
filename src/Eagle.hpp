...
class Eagle
{
	...
	private:
		...
		void asm_alu(const EAGLE_VARIABLE &dst, const EAGLE_VARIABLE &src1,
					 const EAGLE_VARIABLE &src2, const char operator1,
					 const char operator2);
		void asm_falu(const EAGLE_VARIABLE &dst, const EAGLE_VARIABLE &src1,
					  const EAGLE_VARIABLE &src2, const char operator1,
					  const char operator2);

		void asm_bru(const EAGLE_VARIABLE &src1, const EAGLE_VARIABLE &src2,
					 const char operator1, const char operator2, int type,
					 int clabel);
		...
		void asm_alu_65816(const EAGLE_VARIABLE &dst,
						   const EAGLE_VARIABLE &src1,
						   const EAGLE_VARIABLE &src2, const char operator1,
						   const char operator2);

		void asm_bru_65816(const EAGLE_VARIABLE &src1,
						   const EAGLE_VARIABLE &src2, const char operator1,
		...
		void asm_alu_6502(const EAGLE_VARIABLE &dst, const EAGLE_VARIABLE &src1,
						  const EAGLE_VARIABLE &src2, const char operator1,
						  const char operator2);

		void asm_bru_6502(const EAGLE_VARIABLE &src1,
						  const EAGLE_VARIABLE &src2, const char operator1,
						  const char operator2, int type, int clabel);
		...
		void asm_alu_80186(const EAGLE_VARIABLE &dst,
						   const EAGLE_VARIABLE &src1,
						   const EAGLE_VARIABLE &src2, const char operator1,
						   const char operator2);

		void asm_bru_80186(const EAGLE_VARIABLE &src1,
						   const EAGLE_VARIABLE &src2, const char operator1,
						   const char operator2, int type, int clabel);
		...
		void asm_alu_AltairX(const EAGLE_VARIABLE &dst,
							 const EAGLE_VARIABLE &src1,
							 const EAGLE_VARIABLE &src2, const char operator1,
							 const char operator2);
		void asm_falu_AltairX(const EAGLE_VARIABLE &dst,
							  const EAGLE_VARIABLE &src1,
							  const EAGLE_VARIABLE &src2, const char operator1,
							  const char operator2);

		void asm_mul_AltairX(const EAGLE_VARIABLE &dst,
							 const EAGLE_VARIABLE &src1,
							 const EAGLE_VARIABLE &src2);
		void asm_div_AltairX(const EAGLE_VARIABLE &dst,
							 const EAGLE_VARIABLE &src1,
							 const EAGLE_VARIABLE &src2);

		void asm_bru_AltairX(const EAGLE_VARIABLE &src1,
							 const EAGLE_VARIABLE &src2, const char operator1,
							 const char operator2, int type, int clabel);
		void asm_call_jump_AltairX(const EAGLE_VARIABLE &var, int narg,
								   int type);

		bool variable_exist(EAGLE_WORDS tword, EAGLE_VARIABLE &var, int elabel);

		void convertStringToNumber(std::string str, int64_t &result,
								   double &dresult);
